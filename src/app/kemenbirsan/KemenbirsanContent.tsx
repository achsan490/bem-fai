...
const ROLE_ICON: Record<string, React.ReactNode> = {
  "Presiden BEM": <Crown size={16} className="text-amber-500 animate-pulse" />,
      ...
      <div className="relative w-full aspect-[4/5] bg-slate-50 flex items-center justify-center overflow-hidden">
        {member.photo_url ? (
          ...
          <div className="flex flex-col items-center gap-3 opacity-40 group-hover:opacity-60 transition-opacity">
            ...
            <span className="text-xs text-slate-500 font-mono">Foto belum tersedia</span>
          </div>
        )}
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2">
          <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wider text-primary border border-primary/25 bg-white/90 backdrop-blur-md shadow-sm whitespace-nowrap">
        ...
        <section className="pt-32 pb-20 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
            ...
            <motion.h1
              ...
              className="font-display font-bold text-4xl md:text-6xl text-slate-800 leading-tight"
            >
              Pimpinan <span className="text-primary font-extrabold">BEM FAI UNWAHA</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }}
              className="text-slate-600 font-sans text-base md:text-lg mt-4 max-w-2xl mx-auto leading-relaxed"
            >
          ...
          <div className="max-w-7xl mx-auto px-6">
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-center">
                {[...Array(2)].map((_, i) => (
                  <div key={i} className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.015)] animate-pulse">
              ...
              <div className="text-center py-24">
                <User size={56} className="text-primary/40 mx-auto mb-4 animate-pulse" />
                <h3 className="font-display font-bold text-xl text-slate-800 mb-2">Belum Ada Data Pimpinan</h3>
                <p className="text-sm text-slate-500">Data pimpinan dapat ditambahkan melalui Portal Administrasi.</p>
              </div>
            ) : (
